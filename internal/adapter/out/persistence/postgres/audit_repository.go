...
INSERT INTO audit_events (
    ...
    source_ip, request_id, method, path, status_code, details
) VALUES ($1, $2, NULLIF($3, ''), NULLIF($4, ''), NULLIF($5, ''), $6, $7,
          NULLIF($8, ''), NULLIF($9, ''), NULLIF($10, ''), NULLIF($11, ''), $12, $13::jsonb)
`, event.ID, event.OccurredAt, event.ActorType, event.ActorID, event.SessionID, event.Action,
		string(event.Outcome), event.SourceIP, event.RequestID, event.Method, event.Path, event.StatusCode, details)
	if err != nil {
...
SELECT id, occurred_at, COALESCE(actor_type, ''), COALESCE(actor_id, ''), COALESCE(session_id, ''),
       ...
       COALESCE(path, ''), status_code, details
FROM audit_events
...
`, limit, (page-1)*limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&event.Path, &event.StatusCode, &details,
		); err != nil {
